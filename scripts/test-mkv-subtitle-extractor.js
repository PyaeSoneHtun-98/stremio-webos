...
function cuePoint(time,pos){ return el(0xBB,Buffer.concat([el(0xB3,uint(time,4)),el(0xB7,Buffer.concat([el(0xF7,uint(1)),el(0xF1,uint(pos,4))]))])); }
...
function makeFixture(){
  ...
  const dummy=el(0x1C53BB6B,Buffer.concat([cuePoint(359000,0),cuePoint(362000,0)]));
  const p1=info.length+tracks.length+dummy.length,p2=p1+c1.length;
  const cues=el(0x1C53BB6B,Buffer.concat([cuePoint(359000,p1),cuePoint(362000,p2)]));
...
async function main(){
  ...
  try {
    ...
    const result=await extractor.extractWindow(`http://127.0.0.1:${port}/Bleach.mkv`,2,361);
